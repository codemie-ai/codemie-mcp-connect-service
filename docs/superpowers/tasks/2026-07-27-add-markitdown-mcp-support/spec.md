...
**Updated Approach (2026-07-30)**: Install markitdown-mcp into the application's virtual environment instead of system Python:
```dockerfile
...
**Rationale**: 
- Ensures markitdown-mcp uses the same Python environment as the application
- Prevents potential version conflicts between system Python and application venv
- Consistent with best practices for Python virtual environment isolation

This keeps markitdown-mcp separate from Poetry-managed application dependencies, treating it as a standalone tool while maintaining proper environment isolation.

Server invocation: `python -m markitdown_mcp` or package-specific entry point.

## Implementation Details

### 1. Dockerfile Modification

**Location**: Stage 5 (Runtime), after application files are copied (around line 297)

**Final Implementation (2026-07-30)**:
```dockerfile
# Install markitdown-mcp MCP server into the application's venv
RUN /codemie/codemie-mcp-connect/.venv/bin/pip install --no-cache-dir markitdown-mcp==0.0.1a4
```
...
**Rationale**: 
- Uses application venv instead of system Python for proper environment isolation
- Prevents potential version conflicts between system Python and application
- No Poetry lock file changes required
- Installed after application files are in place so venv exists
...
1. Full HTTP cycle - POST to `/bridge`, verify MCP protocol response structure
...
2. **New "Using markitdown-mcp" subsection** with example:
   ```bash
...
|-----------|----------------|
| `markitdown-mcp` installed and configured | Dockerfile pip install in application venv (2026-07-30: fixed to use venv) |
...
**Modified Files**:
- `Dockerfile` - Add pip install markitdown-mcp into application venv (1 line, moved from line 273 to 297)
